...
class AdminCountCheck(Miner):
    ...
    def get_group_of(self, account):
        return self.guid.find_one(
            {"id": account.get('objectGUID')}).get('name')
    ...
    def run(self, options, doc):
        log.warn(
            "You should probably use the SDProp miner, which lists all "
            "accounts who have adminCount=1, then checks results manually.")
        ...
        for account in adminAccounts:
            match = {"$and": [
                {'objectCategory': self.categories.group},
                {"$or":
                 [{"name": {"$regex": account}},
        ...
        for a in self.datatable.find(
                {'$and': [{'objectCategory': self.categories.person},
                          {"adminCount": 1}]}):
        ...
        for account in accountsWithAdminCount:
            if not account.get('objectSid') in adminAccountsMembersSid:
                t.add(
                    ("%s (%s)" % (account.get('name'),
                                  account.get('objectSid')),
                     self.get_group_of(account), account.get('adminCount')))
        t.finished()
